...
// Apply convolutional filter on image data (GPU - device kernel function)
...
int main(int argc, char **argv) {
	...
	*/
	...
	// Initialize host memory for GPU processing
	bmpImageChannel *gpuImageChannel = newBmpImageChannel(im_XSIZE, im_YSIZE);

	int kerDim = 3; // Dimension of currently used kernel
